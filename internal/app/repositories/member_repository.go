...
func NewMember(container *storage.Container) *Member {
	return &Member{
		container: container,
	}
}
...
func (repo *Member) GetMemberByUsernameOrEmail(rctx *requests.RequestContext, username string, email string, member *models.Member) error {
	return repo.container.Config.DB.WithContext(rctx.Ctx).Model(&models.Member{}).
...
func (repo *Member) UsernameExists(rctx *requests.RequestContext, username string) error {
	...
	err := repo.container.Config.DB.WithContext(rctx.Ctx).Model(&models.Member{}).
...
func (repo *Member) Create(rctx *requests.RequestContext, member *models.Member) (uint, error) {
	err := repo.container.Config.DB.WithContext(rctx.Ctx).Create(member).Error
	if err != nil {
		return 0, err
	}
...
func (repo *Member) UpdatePasswordByEmail(rctx *requests.RequestContext, email, newPassword string) error {
	result := repo.container.Config.DB.WithContext(rctx.Ctx).Model(&models.Member{}).
		Where("email = ?", email).
...
func (repo *Member) GetMember(rctx *requests.RequestContext, memberId uint, member *models.Member) error {
	if err := repo.container.Config.DB.WithContext(rctx.Ctx).Where("id = ?", memberId).First(member).Error; err != nil {
		return err
	}
	return nil
}
...
func (repo *Member) Get(rctx *requests.RequestContext, memberId uint) (*responses.MemberResponse, error) {
	...
	query := repo.container.Config.DB.WithContext(rctx.Ctx).Table(member.GetName()+" u").
